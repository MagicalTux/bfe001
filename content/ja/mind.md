...
これには名前があります: **アファンタジア**——自発的に心的イメージを形成できない状態。
VVIQで160点中19点（32点の閾値を大幅に下回る）を記録した。自分でも驚いた——
単にイメージが弱いだけだと思っていたが、実際には心の目はほぼ完全に暗かった。
自分がスペクトルのどこに位置するか、<a href="vviq2.html" class="deep-link">VVIQ2テスト</a>で測定できます。

赤いリンゴを想像してと言われれば、上のヘタや底のくぼみはなんとなく*感じる*。
でも*見える*わけではない。有機的な形は抽象的な空間認識に崩れてしまう。
技術的なもの——コンピュータ、家、錠前の機構——は頭の中で自由に推論できるが、
ワイヤーフレームとして現れる。テクスチャのない構造。見えるとすればの話だが。
